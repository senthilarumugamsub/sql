...
WHERE x.recent_visit_no = 1
ORDER BY customer_id, market_date, transaction_time;
--query that uses this one as a subquery (or temp table) and filters the results to only the customer’s most recent visit
...
SELECT market_date, total_sales, 'Best Day' AS tag
FROM (
    SELECT 
        market_date,
        SUM(quantity * cost_to_customer_per_qty) AS total_sales,
        RANK() OVER (ORDER BY SUM(quantity * cost_to_customer_per_qty) DESC) AS best_rank
    FROM customer_purchases
...
SELECT market_date, total_sales, 'Worst Day' AS tag
FROM (
    SELECT 
        market_date,
        SUM(quantity * cost_to_customer_per_qty) AS total_sales,
        RANK() OVER (ORDER BY SUM(quantity * cost_to_customer_per_qty) ASC) AS worst_rank
    FROM customer_purchases
    GROUP BY market_date
)
WHERE worst_rank = 1;
