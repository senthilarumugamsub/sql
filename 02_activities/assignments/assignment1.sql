...
SELECT v.vendor_name, vba.vendor_id, count(vba.market_date) as booth_rent_count
...
SELECT cp.customer_id, c.customer_last_name, c.customer_first_name,
sum(cp.quantity*cp.cost_to_customer_per_qty) as spend_total
FROM customer_purchases cp
INNER JOIN
customer c
on cp.customer_id = c.customer_id
GROUP by c.customer_id, c.customer_last_name, c.customer_first_name
HAVING spend_total>2000
ORDER by lower(c.customer_last_name), lower(c.customer_first_name);
